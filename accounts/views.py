...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            mail_subject = 'Por favor activa tu cuenta de RychyShop'
            mail_body = render_to_string('accounts/account_verification.html',{
                'user':user,
                'domain':current_site,
            ...
            to_email = email
            send_mail = EmailMessage(mail_subject,mail_body,to=[to_email])
            send_mail.send()
...
def login(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            messages.success(request,'Has iniciado sesión exitósamente')
...
def activate(request,uidb64,token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        messages.success(request,'La activación es inválida')        
...
def resetPassword_validate(request,uidb64,token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        messages.success(request, 'Ahora puedes resetear tu password! :)')
        return redirect('resetPassword')
    else:
        messages.error(request,'El link a expirado')
        return redirect('forgotPassword')
...
def resetPassword(request):
    if request.method == 'POST':
        ...
        if password == confirm_password:
            ...
            user.save()
            messages.success(request,'La contraseña a sido cambiada correctamente')    
            return redirect('login')
        else:
    ...
    if request.method == 'POST':
        ...
        if new_password == confirm_password:
            ...
            if success:
                ...
                user.save()
                messages.success(request,'El password a sido actualizado exitósamente')
                return redirect('change_password')
            else:
                messages.error(request,'Por favor ingrese un password valido')
                return redirect('change_password')
        else:
            messages.error('El password no incide con la confirmación de password')
            return redirect('change_password')
